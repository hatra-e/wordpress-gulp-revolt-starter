...
If you're using a Symlink to link `build` to your `wp-content/themes` directory for local development, please open `src/functions/admin/metaboxes/debug.php` and change the paths on lines 29 and 30 to your own. This will fix a common issue with CMB2 where it tries to load assets from a wrong path.  
